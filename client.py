    ...
    def _call_api(self, endpoint, params=None, query=None, return_response=False, unsigned=False, version='v1'):
        ...
        if params or params == '':
            ...
            if params == '':    # force post if empty string
                ...
                if not unsigned:
                    ...
                    # direct form post
                    post_params = params
                data = compat_urllib_parse.urlencode(post_params).encode('utf-8')
        ...
        try:
            self.logger.debug('RESPONSE: {0:d} {1!s}'.format(response.code, response_content))
                ...
                pass
               # print("ALARM ES WURDE VERMUTLICH EIN NICHT ASCII CHARACTER ENTDECKT UND STRAIGHT UP EINFACH MAL IGNORIERT CAUSE NAJA WIRD SCHO PASSEN")
